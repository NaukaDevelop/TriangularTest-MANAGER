...
export class TranslateErrorService {
  ...
  translateErrorNumber(error: number): string {
    let result: HttpErrorModel = {
      errorCode: 0,
      errorName: 'Unknown',
      errorSpanishTranslation: 'Error desconocido. Revise el servidor',
      errorDef:
        'Error que no se puede traducir o el servidor está desconectado',
    };

    const translation: HttpErrorModel[] = errorList.filter(
      (x: HttpErrorModel) => {
    ...
    return result.errorSpanishTranslation;
  }
}
